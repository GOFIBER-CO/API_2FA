...
async function getPagingGroupProfile(req, res) {
  ...
  if (req.query.search) {
    searchObj = {
      nameProfile: { $regex: ".*" + req.query.search + ".*" },
...
async function deletePasswordGroupProfile(req, res) {
  if (!req.body.password)
    return res
      .status(400)
      .json(new ResponseModel(-3, "Password cannot be left blank!", null));
  try {
    let newPassword = {
      updatedTime: Date.now(),
      password: "",
    ...
    const checkPass = bcrypt.compareSync(
      req.body?.password,
      profileById.password
    );
    if (!checkPass)
      return res
        .status(401)
        .json(new ResponseModel(-1, "Password not match!", null));
    let updatePassword = await Profile.findOneAndUpdate(
      { _id: req.params.id },
      newPassword
    );
...
async function deleteGroupProfile(req, res) {
  if (isValidObjectId(req.params.id)) {
    ...
    res
      .status(404)
      .json(new ResponseModel(404, "Group ProfileId is not valid!", null));
  }
}
...
async function updateGroupProfile(req, res) {
  if (!req.body.nameProfile)
    return res
      .status(400)
      .json(new ResponseModel(-3, "name cannot be left blank!", null));
  try {
    // console.log(req.body,req.params)
    ...
    if (!updateGroupProfile) {
      ...
      let response = new ResponseModel(
        1,
        "update group name profile success!",
        updateGroupProfile
      );
      res.json(response);
    }
